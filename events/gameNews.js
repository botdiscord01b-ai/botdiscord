...
module.exports = {
    ...
    async execute(message) {
        ...
        if (message.content.startsWith('!steam') || message.content.startsWith('!st')) {
            console.log(`[GameNews] เริ่มกระบวนการดึงข้อมูล Steam โดย: ${message.author.tag}`);
            ...
            if (!targetChannel) {
                console.log(`[GameNews Error] ไม่พบห้องเป้าหมาย ID: ${STEAM_CHANNEL_ID}`);
...
async function fetchAndSendSteamNews(channel, type) {
    try {
        console.log(`[GameNews] กำลังเชื่อมต่อข้อมูลประเภท: ${type}...`);
        ...
        try {
            ...
            if (detailData && detailData[randomAppId] && detailData[randomId] && detailData[randomAppId].success) {
            ...
            console.log(`[GameNews Info] ข้ามการดึงราคา ใช้ค่าสำรองแทน`);
        }
        ...
        if (duplicateMessages.size > 0) {
            console.log(`[GameNews] ลบโพสต์เกมซ้ำของ ${gameName}...`);
        ...
        const button = new ButtonBuilder()
            ...
            .setUrl(storeUrl); // ใช้ตัวแอลพิมพ์เล็กถูกต้อง
        ...
        console.log(`[GameNews] โพสต์เกม ${gameName} สำเร็จเรียบร้อย!`);
    } catch (error) {
        console.error(`[GameNews Error] เกิดข้อผิดพลาด:`, error.message);
    }
}
